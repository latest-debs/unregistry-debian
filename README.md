...
[![Release](https://img.shields.io/github/v/release/latest-debs/unregistry-debian)](https://github.com/latest-debs/unregistry-debian/releases)
[![Build](https://github.com/latest-debs/unregistry-debian/actions/workflows/release.yml/badge.svg)](../../actions)

Push docker images directly to remote servers without an external registry — packaged for Debian as part of [latest-debs](https://github.com/latest-debs).
...
Or download a `.deb` from the [Releases](https://github.com/latest-debs/unregistry-debian/releases) page:

```sh
sudo apt install ./unregistry_*.deb
```

## Verify

```sh
apt-cache policy unregistry
unregistry --version
```
...
Run the [Build unregistry for Debian](../../actions) workflow on GitHub with the
...
For issues with unregistry itself, see
